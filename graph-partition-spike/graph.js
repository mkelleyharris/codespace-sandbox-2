...
export class Graph {
  ...
  words(){
    ...
    for (const node of this.nodes) {
      ...
      for (const prop in node.props) {
        result.push(prop)
        result.push(node.props[prop])
      }
    }
    for (const rel of this.rels) {
      result.push(rel.type)
      for (const prop in rel.props) {
        result.push(prop)
        result.push(rel.props[prop])
      }
    }
    return result.sort()
  }
  ...
  */
  ...
  getSubGraph(startingNode) {
    // TODO use startingNode to start location
    return new Graph(this.nodes, this.rels)
  }
